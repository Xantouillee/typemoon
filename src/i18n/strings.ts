// Lightweight, dependency-free UI localisation. The UI language follows the
// selected typing language, so switching to Français also translates the chrome.

export type Dict = Record<string, string>;

const en: Dict = {
  tagline: 'a typing practice for people who love words',
  time: 'time',
  words: 'words',
  quote: 'quote',
  daily: 'daily page',
  zen: 'zen',
  punctuation: 'punctuation',
  numbers: 'numbers',
  restart: 'restart',
  next: 'next',
  clickToFocus: 'click to resume',
  wpm: 'wpm',
  accuracy: 'accuracy',
  raw: 'raw',
  consistency: 'consistency',
  time_stat: 'time',
  characters: 'characters',
  again: 'again',
  newTest: 'new test',
  history: 'history',
  practice: 'practice',
  arcade: 'arcade',
  mode: 'mode',
  todaysPage: "today's page",
  dailyStreakN: '{n}-day streak',
  dailyBestN: 'best {n}',
  dailyDone: 'Done today',
  dailyKeep: 'Come back tomorrow to keep your streak.',
  dailyStart: "Finish today's page to start a streak.",
  dailyRankToday: "#{rank} on today's page",
  dailyOnFire: 'On a roll — {n} days and counting.',
  bestWpm: 'best wpm',
  avgWpm: 'avg wpm',
  avgAcc: 'avg accuracy',
  tests: 'tests',
  noHistory: 'No runs yet. Your finished tests will appear here.',
  // accounts & leaderboard
  leaderboard: 'leaderboard',
  account: 'account',
  signIn: 'sign in',
  signOut: 'sign out',
  rename: 'rename',
  showOnBoard: 'Show me on the leaderboard',
  boardHiddenHint: "You're hidden from the board — your runs still count anonymously.",
  save: 'save',
  signInBlurb: 'Sign in to save your stats across devices and climb the leaderboard.',
  continueGoogle: 'Continue with Google',
  continueGitHub: 'Continue with GitHub',
  signInToSync: 'Sign in to sync your stats across devices.',
  syncedDevices: 'synced across your devices',
  streak: 'streak',
  days: 'days',
  keysTapped: 'keys tapped',
  timeTyped: 'time typed',
  daysActive: 'days active',
  longestStreak: 'longest streak',
  favourite: 'favourite',
  today: 'today',
  thisWeek: 'this week',
  thisMonth: 'this month',
  allTime: 'all-time',
  allModes: 'all modes',
  allLangs: 'all languages',
  signInToCompete: 'Sign in to claim your spot on the board.',
  leaderboardEmpty: 'No runs in this window yet. Be the first.',
  leaderboardWarming: 'The leaderboard is warming up — check back soon.',
  you: 'you',
  slowestKeys: 'slowest keys',
  keyboardSound: 'keyboard sound',
  allSettings: 'all settings',
  settings: 'settings',
  tier_tactile: 'tactile',
  tier_musical: 'musical',
  tier_melody: 'melody',
  tier_joke: 'joke',
  soundVoice: 'voice',
  proof: 'proofread',
  finished: 'finished',
  hideBackdrop: 'hide the backdrop',
  showBackdrop: 'show the backdrop',
  verdictFirst: 'First run at {mode}. This is the mark to beat.',
  verdictBest: 'Personal best — {delta} above your old best of {best}.',
  verdictRank: 'Your {rank} best at {mode}',
  verdictAbove: '{delta} above your recent average',
  verdictBelow: '{delta} below your recent average',
  verdictLevel: 'right on your recent average',
  percentile: 'Faster than {pct}% of runs at {mode}.',
  endedEarly: 'ended early',
  endedMaster: 'Master difficulty: a single wrong key ends the run.',
  endedExpert: 'Expert difficulty: a word went through with a mistake in it.',
  endedNote: 'Everything you typed up to that point still counts.',
  blockedLetter: 'Fix that letter to carry on.',
  blockedWord: 'Fix the mistake in this word to carry on.',
  inkRushBlurb: 'Chips × Multiplier. Every clean word feeds the multiplier — one mistake halves it and cracks a heart. No backspace. Lose three hearts and the run is over.',
  anthologySub: '6-page roguelite run · pick Quills · beat the final boss',
  scoreAttack: 'score attack',
  newBadge: 'new',
  best: 'best',
  noRunsYet: 'no runs yet',
  pagesLabel: 'pages',
  peakMult: 'peak mult',
  quillsLabel: 'quills',
  yourQuills: 'your quills',
  newRun: 'New run',
  backToArcade: 'Back to arcade',
  pressEnterNewRun: 'for a new run',
  footer: 'Typemoon · public-domain passages via Project Gutenberg · built for the love of letters',
  notesRead: '{n} notes read',
  notesRejected: 'not understood: {list}',
  'midi_not-midi': 'That is not a MIDI file.',
  'midi_no-notes': 'No notes found in that file.',
  midi_unreadable: 'That file could not be read.',
  startTyping: 'Start typing',
  stopRun: 'Stop',
  tapToStart: 'Tap the words, then type',
  tapToResume: 'Tap to resume',
  newWords: 'New words',
  mobileTitle: 'Type',
  theme: 'theme',
  volume: 'volume',
  textSize: 'text size',
  share: 'share',
  shareScore: 'share',
  copyLink: 'copy link',
  copied: 'copied',
  downloadCard: 'download card',
  beatIt: 'Try to beat it',
  canYouBeat: 'Can you beat this?',
  noScore: 'No score to show here.',
};

const fr: Dict = {
  tagline: 'un entraînement de frappe pour les amoureux des mots',
  time: 'temps',
  words: 'mots',
  quote: 'citation',
  daily: 'page du jour',
  zen: 'zen',
  punctuation: 'ponctuation',
  numbers: 'chiffres',
  restart: 'recommencer',
  next: 'suivant',
  clickToFocus: 'cliquez pour reprendre',
  wpm: 'mpm',
  accuracy: 'précision',
  raw: 'brut',
  consistency: 'régularité',
  time_stat: 'temps',
  characters: 'caractères',
  again: 'encore',
  newTest: 'nouveau test',
  history: 'historique',
  practice: 'pratique',
  arcade: 'arcade',
  mode: 'mode',
  todaysPage: 'la page du jour',
  dailyStreakN: 'série de {n} jours',
  dailyBestN: 'record {n}',
  dailyDone: 'Fait aujourd’hui',
  dailyKeep: 'Revenez demain pour garder votre série.',
  dailyStart: 'Terminez la page du jour pour lancer une série.',
  dailyRankToday: '#{rank} sur la page du jour',
  dailyOnFire: 'En feu — {n} jours d’affilée.',
  bestWpm: 'meilleur mpm',
  avgWpm: 'mpm moyen',
  avgAcc: 'précision moy.',
  tests: 'tests',
  noHistory: 'Aucune session. Vos tests terminés apparaîtront ici.',
  // comptes & classement
  leaderboard: 'classement',
  account: 'compte',
  signIn: 'se connecter',
  signOut: 'se déconnecter',
  rename: 'renommer',
  showOnBoard: 'Afficher mon nom au classement',
  boardHiddenHint: 'Vous êtes masqué du classement — vos sessions comptent toujours de façon anonyme.',
  save: 'enregistrer',
  signInBlurb: 'Connectez-vous pour sauvegarder vos stats sur tous vos appareils et grimper au classement.',
  continueGoogle: 'Continuer avec Google',
  continueGitHub: 'Continuer avec GitHub',
  signInToSync: 'Connectez-vous pour synchroniser vos stats entre appareils.',
  syncedDevices: 'synchronisé sur vos appareils',
  streak: 'série',
  days: 'jours',
  keysTapped: 'touches frappées',
  timeTyped: 'temps de frappe',
  daysActive: 'jours actifs',
  longestStreak: 'plus longue série',
  favourite: 'favori',
  today: "aujourd'hui",
  thisWeek: 'cette semaine',
  thisMonth: 'ce mois',
  allTime: 'toujours',
  allModes: 'tous les modes',
  allLangs: 'toutes les langues',
  signInToCompete: 'Connectez-vous pour prendre votre place au classement.',
  leaderboardEmpty: 'Aucune session sur cette période. Soyez le premier.',
  leaderboardWarming: 'Le classement se prépare — revenez bientôt.',
  you: 'vous',
  slowestKeys: 'touches les plus lentes',
  keyboardSound: 'son du clavier',
  allSettings: 'tous les réglages',
  settings: 'réglages',
  tier_tactile: 'tactile',
  tier_musical: 'musical',
  tier_melody: 'mélodie',
  tier_joke: 'blague',
  soundVoice: 'voix',
  proof: 'relecture',
  finished: 'terminé',
  hideBackdrop: 'masquer le décor',
  showBackdrop: 'afficher le décor',
  verdictFirst: 'Première session en {mode}. Voilà la marque à battre.',
  verdictBest: 'Record personnel — {delta} de plus que votre ancien record de {best}.',
  verdictRank: 'Votre {rank} meilleure session en {mode}',
  verdictAbove: '{delta} au-dessus de votre moyenne récente',
  verdictBelow: '{delta} en dessous de votre moyenne récente',
  verdictLevel: 'pile sur votre moyenne récente',
  percentile: 'Plus rapide que {pct}% des sessions en {mode}.',
  endedEarly: 'arrêtée avant la fin',
  endedMaster: 'Difficulté Master : une seule touche fausse met fin à la session.',
  endedExpert: 'Difficulté Expert : un mot est passé avec une faute dedans.',
  endedNote: 'Tout ce que vous avez tapé jusque-là compte quand même.',
  blockedLetter: 'Corrigez cette lettre pour continuer.',
  blockedWord: 'Corrigez la faute dans ce mot pour continuer.',
  inkRushBlurb: 'Jetons × Multiplicateur. Chaque mot sans faute nourrit le multiplicateur — une faute le divise par deux et fêle un cœur. Pas de retour arrière. Trois cœurs perdus et la partie s’arrête.',
  anthologySub: 'partie roguelite en 6 pages · choisissez vos Plumes · battez le boss final',
  scoreAttack: 'attaque de score',
  newBadge: 'nouveau',
  best: 'record',
  noRunsYet: 'aucune partie',
  pagesLabel: 'pages',
  peakMult: 'mult. max',
  quillsLabel: 'plumes',
  yourQuills: 'vos plumes',
  newRun: 'Nouvelle partie',
  backToArcade: 'Retour à l’arcade',
  pressEnterNewRun: 'pour une nouvelle partie',
  footer: 'Typemoon · textes du domaine public via le Projet Gutenberg · fait par amour des lettres',
  notesRead: '{n} notes lues',
  notesRejected: 'non compris : {list}',
  'midi_not-midi': 'Ce n’est pas un fichier MIDI.',
  'midi_no-notes': 'Aucune note trouvée dans ce fichier.',
  midi_unreadable: 'Ce fichier n’a pas pu être lu.',
  startTyping: 'Commencer',
  stopRun: 'Arrêter',
  tapToStart: 'Touchez le texte, puis tapez',
  tapToResume: 'Touchez pour reprendre',
  newWords: 'Nouveaux mots',
  mobileTitle: 'Taper',
  theme: 'thème',
  volume: 'volume',
  textSize: 'taille du texte',
  share: 'partager',
  shareScore: 'partager',
  copyLink: 'copier le lien',
  copied: 'copié',
  downloadCard: 'télécharger la carte',
  beatIt: 'À toi de battre ça',
  canYouBeat: 'Peux-tu faire mieux ?',
  noScore: 'Aucun score à afficher ici.',
};

const es: Dict = {
  tagline: 'una práctica de mecanografía para los amantes de las palabras',
  time: 'tiempo',
  words: 'palabras',
  quote: 'cita',
  daily: 'página del día',
  zen: 'zen',
  punctuation: 'puntuación',
  numbers: 'números',
  restart: 'reiniciar',
  next: 'siguiente',
  clickToFocus: 'clic para continuar',
  wpm: 'ppm',
  accuracy: 'precisión',
  raw: 'bruto',
  consistency: 'regularidad',
  time_stat: 'tiempo',
  characters: 'caracteres',
  again: 'otra vez',
  newTest: 'nueva prueba',
  history: 'historial',
  practice: 'práctica',
  arcade: 'arcade',
  mode: 'modo',
  todaysPage: 'la página de hoy',
  dailyStreakN: 'racha de {n} días',
  dailyBestN: 'récord {n}',
  dailyDone: 'Hecho hoy',
  dailyKeep: 'Vuelve mañana para mantener tu racha.',
  dailyStart: 'Termina la página de hoy para empezar una racha.',
  dailyRankToday: '#{rank} en la página de hoy',
  dailyOnFire: 'En racha — {n} días seguidos.',
  bestWpm: 'mejor ppm',
  avgWpm: 'ppm medio',
  avgAcc: 'precisión med.',
  tests: 'pruebas',
  noHistory: 'Aún no hay sesiones. Tus pruebas terminadas aparecerán aquí.',
  // cuentas y clasificación
  leaderboard: 'clasificación',
  account: 'cuenta',
  signIn: 'iniciar sesión',
  signOut: 'cerrar sesión',
  rename: 'renombrar',
  showOnBoard: 'Mostrarme en la clasificación',
  boardHiddenHint: 'Estás oculto de la clasificación — tus sesiones siguen contando de forma anónima.',
  save: 'guardar',
  signInBlurb: 'Inicia sesión para guardar tus estadísticas en todos tus dispositivos y subir en la clasificación.',
  continueGoogle: 'Continuar con Google',
  continueGitHub: 'Continuar con GitHub',
  signInToSync: 'Inicia sesión para sincronizar tus estadísticas entre dispositivos.',
  syncedDevices: 'sincronizado en tus dispositivos',
  streak: 'racha',
  days: 'días',
  keysTapped: 'teclas pulsadas',
  timeTyped: 'tiempo escrito',
  daysActive: 'días activos',
  longestStreak: 'racha más larga',
  favourite: 'favorito',
  today: 'hoy',
  thisWeek: 'esta semana',
  thisMonth: 'este mes',
  allTime: 'histórico',
  allModes: 'todos los modos',
  allLangs: 'todos los idiomas',
  signInToCompete: 'Inicia sesión para reclamar tu lugar en la clasificación.',
  leaderboardEmpty: 'Aún no hay sesiones en este periodo. Sé el primero.',
  leaderboardWarming: 'La clasificación se está preparando — vuelve pronto.',
  you: 'tú',
  slowestKeys: 'teclas más lentas',
  keyboardSound: 'sonido del teclado',
  allSettings: 'todos los ajustes',
  settings: 'ajustes',
  tier_tactile: 'táctil',
  tier_musical: 'musical',
  tier_melody: 'melodía',
  tier_joke: 'broma',
  soundVoice: 'voz',
  proof: 'revisión',
  finished: 'terminado',
  hideBackdrop: 'ocultar el fondo',
  showBackdrop: 'mostrar el fondo',
  verdictFirst: 'Primera sesión en {mode}. Esta es la marca a batir.',
  verdictBest: 'Récord personal — {delta} por encima de tu récord anterior de {best}.',
  verdictRank: 'Tu {rank} mejor sesión en {mode}',
  verdictAbove: '{delta} por encima de tu media reciente',
  verdictBelow: '{delta} por debajo de tu media reciente',
  verdictLevel: 'justo en tu media reciente',
  percentile: 'Más rápido que el {pct}% de las sesiones en {mode}.',
  endedEarly: 'terminada antes de tiempo',
  endedMaster: 'Dificultad Master: una sola tecla equivocada termina la sesión.',
  endedExpert: 'Dificultad Experto: una palabra pasó con un error dentro.',
  endedNote: 'Todo lo que escribiste hasta ese punto sigue contando.',
  blockedLetter: 'Corrige esa letra para continuar.',
  blockedWord: 'Corrige el error de esta palabra para continuar.',
  inkRushBlurb: 'Fichas × Multiplicador. Cada palabra limpia alimenta el multiplicador — un error lo parte por la mitad y agrieta un corazón. Sin retroceso. Pierde tres corazones y se acabó.',
  anthologySub: 'partida roguelite de 6 páginas · elige Plumas · vence al jefe final',
  scoreAttack: 'ataque de puntuación',
  newBadge: 'nuevo',
  best: 'récord',
  noRunsYet: 'sin partidas',
  pagesLabel: 'páginas',
  peakMult: 'mult. máx',
  quillsLabel: 'plumas',
  yourQuills: 'tus plumas',
  newRun: 'Nueva partida',
  backToArcade: 'Volver al arcade',
  pressEnterNewRun: 'para una nueva partida',
  footer: 'Typemoon · pasajes de dominio público vía Proyecto Gutenberg · hecho por amor a las letras',
  notesRead: '{n} notas leídas',
  notesRejected: 'no entendido: {list}',
  'midi_not-midi': 'Eso no es un archivo MIDI.',
  'midi_no-notes': 'No se han encontrado notas en ese archivo.',
  midi_unreadable: 'No se ha podido leer ese archivo.',
  startTyping: 'Empezar',
  stopRun: 'Parar',
  tapToStart: 'Toca el texto y escribe',
  tapToResume: 'Toca para continuar',
  newWords: 'Nuevas palabras',
  mobileTitle: 'Escribir',
  theme: 'tema',
  volume: 'volumen',
  textSize: 'tamaño del texto',
  share: 'compartir',
  shareScore: 'compartir',
  copyLink: 'copiar enlace',
  copied: 'copiado',
  downloadCard: 'descargar tarjeta',
  beatIt: 'Supéralo',
  canYouBeat: '¿Puedes superarlo?',
  noScore: 'No hay puntuación que mostrar.',
};

const de: Dict = {
  tagline: 'ein Tipptraining für alle, die Wörter lieben',
  time: 'Zeit',
  words: 'Wörter',
  quote: 'Zitat',
  daily: 'Seite des Tages',
  zen: 'Zen',
  punctuation: 'Satzzeichen',
  numbers: 'Zahlen',
  restart: 'neu starten',
  next: 'weiter',
  clickToFocus: 'klicken zum Fortsetzen',
  wpm: 'WpM',
  accuracy: 'Genauigkeit',
  raw: 'roh',
  consistency: 'Gleichmaß',
  time_stat: 'Zeit',
  characters: 'Zeichen',
  again: 'nochmal',
  newTest: 'neuer Test',
  history: 'Verlauf',
  practice: 'Übung',
  arcade: 'Arcade',
  mode: 'Modus',
  todaysPage: 'die heutige Seite',
  dailyStreakN: '{n}-Tage-Serie',
  dailyBestN: 'Rekord {n}',
  dailyDone: 'Heute erledigt',
  dailyKeep: 'Komm morgen wieder, um deine Serie zu halten.',
  dailyStart: 'Beende die heutige Seite, um eine Serie zu starten.',
  dailyRankToday: '#{rank} auf der heutigen Seite',
  dailyOnFire: 'In Fahrt — {n} Tage in Folge.',
  bestWpm: 'bestes WpM',
  avgWpm: 'Ø WpM',
  avgAcc: 'Ø Genauigkeit',
  tests: 'Tests',
  noHistory: 'Noch keine Läufe. Deine beendeten Tests erscheinen hier.',
  // Konten & Bestenliste
  leaderboard: 'Bestenliste',
  account: 'Konto',
  signIn: 'anmelden',
  signOut: 'abmelden',
  rename: 'umbenennen',
  showOnBoard: 'Mich in der Bestenliste zeigen',
  boardHiddenHint: 'Du bist aus der Bestenliste ausgeblendet — deine Läufe zählen weiterhin anonym.',
  save: 'speichern',
  signInBlurb: 'Melde dich an, um deine Statistiken geräteübergreifend zu speichern und in der Bestenliste aufzusteigen.',
  continueGoogle: 'Mit Google fortfahren',
  continueGitHub: 'Mit GitHub fortfahren',
  signInToSync: 'Melde dich an, um deine Statistiken geräteübergreifend zu synchronisieren.',
  syncedDevices: 'geräteübergreifend synchronisiert',
  streak: 'Serie',
  days: 'Tage',
  keysTapped: 'Tasten getippt',
  timeTyped: 'Tippzeit',
  daysActive: 'aktive Tage',
  longestStreak: 'längste Serie',
  favourite: 'Favorit',
  today: 'heute',
  thisWeek: 'diese Woche',
  thisMonth: 'dieser Monat',
  allTime: 'gesamt',
  allModes: 'alle Modi',
  allLangs: 'alle Sprachen',
  signInToCompete: 'Melde dich an, um deinen Platz in der Bestenliste zu sichern.',
  leaderboardEmpty: 'Noch keine Läufe in diesem Zeitraum. Sei der Erste.',
  leaderboardWarming: 'Die Bestenliste wird vorbereitet — schau bald wieder vorbei.',
  you: 'du',
  slowestKeys: 'langsamste Tasten',
  keyboardSound: 'Tastaturklang',
  allSettings: 'alle Einstellungen',
  settings: 'Einstellungen',
  tier_tactile: 'taktil',
  tier_musical: 'musikalisch',
  tier_melody: 'Melodie',
  tier_joke: 'Scherz',
  soundVoice: 'Klangfarbe',
  proof: 'Korrektur',
  finished: 'fertig',
  hideBackdrop: 'Hintergrund ausblenden',
  showBackdrop: 'Hintergrund einblenden',
  verdictFirst: 'Erster Lauf bei {mode}. Das ist die Marke, die es zu schlagen gilt.',
  verdictBest: 'Persönliche Bestleistung — {delta} über deiner alten Bestmarke von {best}.',
  verdictRank: 'Dein {rank} bester Lauf bei {mode}',
  verdictAbove: '{delta} über deinem jüngsten Schnitt',
  verdictBelow: '{delta} unter deinem jüngsten Schnitt',
  verdictLevel: 'genau auf deinem jüngsten Schnitt',
  percentile: 'Schneller als {pct}% der Läufe bei {mode}.',
  endedEarly: 'vorzeitig beendet',
  endedMaster: 'Schwierigkeit Master: eine einzige falsche Taste beendet den Lauf.',
  endedExpert: 'Schwierigkeit Experte: ein Wort ging mit einem Fehler durch.',
  endedNote: 'Alles, was du bis dahin getippt hast, zählt weiterhin.',
  blockedLetter: 'Korrigiere diesen Buchstaben, um weiterzukommen.',
  blockedWord: 'Korrigiere den Fehler in diesem Wort, um weiterzukommen.',
  inkRushBlurb: 'Chips × Multiplikator. Jedes fehlerfreie Wort füttert den Multiplikator — ein Fehler halbiert ihn und bricht ein Herz. Keine Rücktaste. Drei Herzen verloren, und der Lauf ist vorbei.',
  anthologySub: 'Roguelite-Lauf über 6 Seiten · wähle Federn · besiege den Endgegner',
  scoreAttack: 'Punktejagd',
  newBadge: 'neu',
  best: 'Bestwert',
  noRunsYet: 'noch keine Läufe',
  pagesLabel: 'Seiten',
  peakMult: 'max. Mult.',
  quillsLabel: 'Federn',
  yourQuills: 'deine Federn',
  newRun: 'Neuer Lauf',
  backToArcade: 'Zurück zur Arcade',
  pressEnterNewRun: 'für einen neuen Lauf',
  footer: 'Typemoon · gemeinfreie Textstellen via Project Gutenberg · aus Liebe zu den Buchstaben',
  notesRead: '{n} Noten gelesen',
  notesRejected: 'nicht verstanden: {list}',
  'midi_not-midi': 'Das ist keine MIDI-Datei.',
  'midi_no-notes': 'In dieser Datei wurden keine Noten gefunden.',
  midi_unreadable: 'Diese Datei konnte nicht gelesen werden.',
  startTyping: 'Starten',
  stopRun: 'Stopp',
  tapToStart: 'Text antippen, dann tippen',
  tapToResume: 'Zum Fortfahren tippen',
  newWords: 'Neue Wörter',
  mobileTitle: 'Tippen',
  theme: 'Thema',
  volume: 'Lautstärke',
  textSize: 'Textgröße',
  share: 'teilen',
  shareScore: 'teilen',
  copyLink: 'Link kopieren',
  copied: 'kopiert',
  downloadCard: 'Karte laden',
  beatIt: 'Schlag das',
  canYouBeat: 'Schaffst du mehr?',
  noScore: 'Kein Ergebnis zum Anzeigen.',
};

/** Exported so a test can prove no language has silently fallen behind English. */
export const DICTS: Record<string, Dict> = { en, fr, es, de };

export function t(lang: string, key: string): string {
  return DICTS[lang]?.[key] ?? en[key] ?? key;
}

/** `t` with `{placeholders}` filled in. */
export function tf(lang: string, key: string, vars: Record<string, string | number>): string {
  return t(lang, key).replace(/\{(\w+)\}/g, (m, name) =>
    name in vars ? String(vars[name]) : m,
  );
}

/**
 * "3rd" / "3e" / "3.º" / "3." — languages disagree about ordinals, and getting
 * this wrong is the sort of small thing that makes an interface feel foreign.
 */
export function ordinal(lang: string, n: number): string {
  if (lang === 'fr') return n === 1 ? '1re' : `${n}e`;
  if (lang === 'es') return `${n}.ª`;
  if (lang === 'de') return `${n}.`;
  const rem100 = n % 100;
  if (rem100 >= 11 && rem100 <= 13) return `${n}th`;
  const suffix = { 1: 'st', 2: 'nd', 3: 'rd' }[n % 10] ?? 'th';
  return `${n}${suffix}`;
}
